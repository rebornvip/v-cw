...
<script>
    ...
    export default {
      ...
      methods: {
          login(ev) {
  //          sessionStorage.setItem('user', JSON.stringify({username:"admin",password:"123123"}));
              if(this.account!='' && this.password!=''){
                  var exdate = new Date();
                  let expireDays = 1000 * 60 * 60 * 24 * 15;
                  this.setCookie('session',{account:this.account}, expireDays);
                  var _this =this;
                  var loginParams = { username: this.account, password: this.password };
                  requestLogin(loginParams).then(data => {
                      ...
                      if (code !== 200) {
                          ...
                          /*vuex*/
//                          this.$store.commit('updateUserInfo', JSON.stringify(user));
                          this.isLoading=true;
...
//                  this.$store.state.user_name =this.account;



              }else{
                  this.$message({
                      message: '用户名或密码错误，请核对后在登录！',
                      type: 'error'
                  });
              }
        }
